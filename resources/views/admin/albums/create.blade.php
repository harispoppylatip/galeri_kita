...
@section('title', 'Buat Album — Admin')
            ...
            <form action="{{ route('admin.albums.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="3" placeholder="Ceritakan tentang album ini..."
                        class="w-full rounded-xl border border-brown-light/40 bg-cream-light px-4 py-2.5 text-sm text-brown-dark placeholder:text-brown-medium/60 focus:border-brown-primary focus:outline-none focus:ring-2 focus:ring-brown-primary/30 resize-none">{{ old('deskripsi') }}</textarea>
                </div>

                {{-- Cover --}}
                <div>
                    <label for="cover" class="mb-2 block text-sm font-semibold text-brown-dark">Cover Album
                        <span class="font-normal text-brown-medium">(opsional, otomatis dari foto pertama)</span>
                    </label>
